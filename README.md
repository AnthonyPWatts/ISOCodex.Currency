...
This package is stable at `1.0.1`. The core API is framework-agnostic, optional integrations are split into separate packages, and EF Core persistence helpers live in an optional package.
...
```bash
dotnet add package ISOCodex.Currency.Json.SystemTextJson --version 1.0.1
```
...
```bash
dotnet add package ISOCodex.Currency.Json.NewtonsoftJson --version 1.0.1
```
...
```bash
dotnet add package ISOCodex.Currency.Addressing --version 1.0.1
```
...
```bash
dotnet add package ISOCodex.Currency.Exchange.Abstractions --version 1.0.1
```
...
```bash
dotnet add package ISOCodex.Currency.Dapper --version 1.0.1
```
...
```bash
dotnet add package ISOCodex.Currency.AspNetCore --version 1.0.1
```
...
The current rules are `ISOCCUR001`, which warns on `default(Money)` and `default` literals converted to `Money`; `ISOCCUR002`, which warns on `default(CurrencyCode)` and `default` literals converted to `CurrencyCode`; `ISOCCUR003`, which warns when `MoneyValidationResult` or `MoneyParseResult` values are ignored; and `ISOCCUR005`, which warns when symbol parsing is enabled without an expected currency. Use `Money.Zero(currency)`, `Money.Of(amount, currency)`, `CurrencyCode.Parse(...)`, known static currency codes, explicit `Succeeded`/failure-reason checks, and expected-currency symbol parsing instead.
...
If a local machine has newer compatible runtimes but not the .NET 9 runtime, use `pwsh ./eng/smoke-test-package.ps1 -Version 1.0.1 -UseMajorRollForward` for the smoke test. This is a local workaround; CI installs .NET 9 and .NET 10 explicitly.
...
pwsh ./eng/pack-packages.ps1 -Configuration Release -OutputPath artifacts -Version 1.0.1
pwsh ./eng/smoke-test-package.ps1 -Version 1.0.1
```

## Currency data workflow

The `1.0.1` registry is generated from a derived JSON snapshot built from checked-in SIX ISO 4217 and Unicode CLDR source files, pinned by `data/source/currency-data.manifest.json`:
