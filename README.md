...
This package is stable at `1.0.2`. The core API is framework-agnostic, optional integrations are split into separate packages, and EF Core persistence helpers live in an optional package.
...
```bash
dotnet add package ISOCodex.Currency.Json.SystemTextJson --version 1.0.2
```
...
```bash
dotnet add package ISOCodex.Currency.Json.NewtonsoftJson --version 1.0.2
```
...
```bash
dotnet add package ISOCodex.Currency.Countries --version 1.0.2
```
...
```bash
dotnet add package ISOCodex.Currency.Dapper --version 1.0.2
```
...
```bash
dotnet add package ISOCodex.Currency.Validation --version 1.0.2
```
...
If a local machine has newer compatible runtimes but not the .NET 9 runtime, use `pwsh ./eng/smoke-test-package.ps1 -Version 1.0.2 -UseMajorRollForward` for the smoke test. This is a local workaround; CI installs .NET 9 and .NET 10 explicitly.
...
pwsh ./eng/pack-packages.ps1 -Configuration Release -OutputPath artifacts -Version 1.0.2
pwsh ./eng/smoke-test-package.ps1 -Version 1.0.2
```

## Currency data workflow

The `1.0.2` registry is generated from a derived JSON snapshot built from checked-in SIX ISO 4217 and Unicode CLDR source files, pinned by `data/source/currency-data.manifest.json`:
